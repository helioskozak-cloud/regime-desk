...
A market regime dashboard hosted on GitHub Pages. (It was self-evolving until
2026-09-21, when the Claude-API improver was retired; changes are made by hand now.)

## Structure

```
docs/index.html          — single self-contained dashboard (no CDN, no fetch)
build/build.py           — daily orchestrator: refresh → validate → promote
build/snapshot_builder.py — converts data/ files to window.SNAPSHOT JS block
build/improver.py        — RETIRED 2026-09-21, kept for reference, not called
...
wishlist.md              — ideas list (the agent that read it is retired)
...
4. `build/validator.py` checks the result; rolls back if anything fails
5. The new HTML is committed and GitHub Pages redeploys automatically
